            ...
            {project.languages && project.languages.length > 0 && (
              <div className="flex flex-wrap gap-2 sm:justify-end">
                {project.languages.map((lang) => (
                  <div key={lang} className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-full px-3 py-1.5 text-sm md:text-base">
          ...
          <div className="flex flex-col md:flex-row gap-8 mb-6">
            ...
            <div className="md:w-1/2 flex-grow">
              ...
              <div className="flex flex-wrap gap-4">
                {project.demoUrl && (
                  <Link href={project.demoUrl} target="_blank" rel="noopener noreferrer" className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded-full inline-block transition-all duration-300 transform hover:scale-105">
                    Ouvrir le projet
                  </Link>
                )}
                {project.githubUrl && (
                  <Link href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="bg-gray-800 dark:bg-gray-600 hover:bg-gray-900 dark:hover:bg-gray-700 text-white font-bold py-2 px-6 rounded-full inline-flex items-center transition-all duration-300 transform hover:scale-105">
          ...
          {project.presentationImages && project.presentationImages.length > 0 && (
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {project.presentationImages.map((img, index) => (
                  img && (
                    <motion.img 
                      key={index} 
                      src={img} 
